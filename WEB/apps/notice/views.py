...
file_handler = logging.FileHandler('static/adminLog/notice.log')
...
def noticeEdit(request, board_id):
    if request.method == 'GET':
        ...
        logger.info(f'공지사항 수정 : {request.session["user"]} [{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]')
...
class noticeWriteSubmit(View):
    def post(self, request):
        ...
        logger.info(f'공지사항 작성 : {request.session["user"]} [{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}]')
